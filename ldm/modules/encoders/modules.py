...
from transformers import T5Tokenizer, ViTModel, ViTMAEModel, CLIPVisionModel, T5EncoderModel, CLIPTokenizer, CLIPTextModel
...
class FrozenViTMAE_layers(AbstractEncoder):
    """Uses the ViT_MAE encoder for controlnet (from huggingface)"""
    def __init__(self, version="facebook/vit-mae-base", device="cuda", layer=12, mask_ratio=0., freeze=True): 
        super().__init__()
        self.transformer = ViTMAEModel.from_pretrained(version)
        self.layer = layer
        self.device = device
...
class FrozenViTMAE_all(AbstractEncoder):
    """Uses the ViT_MAE encoder for controlnet (from huggingface)"""
    def __init__(self, version="facebook/vit-mae-base", device="cuda", mask_ratio=0., freeze=True): 
        super().__init__()
        self.transformer = ViTMAEModel.from_pretrained(version)
